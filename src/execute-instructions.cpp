...
void print_unimplemented(std::string_view label)
{
  std::cout << "Unimplemented instruction: " << label << '\n';
}
} // namespace
...
void Chip8::execute(DecodeTypes::List decode_list, Device &device, AppRenderer &renderer)
{
  ...
  std::visit(visitor{
      [&device](DecodeTypes::ClearDisplay) -> auto {
        device.screen_buffer_.clear_buffer();
      },

      [](DecodeTypes::ReturnFromSubroutine) -> auto {
        print_unimplemented("return_from_subroutine");
      },
      ...
      [](DecodeTypes::RegisterToRegisterArith) -> auto {
        print_unimplemented("register_to_register_arith");
      },
